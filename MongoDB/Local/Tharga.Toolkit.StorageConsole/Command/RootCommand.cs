...
namespace Tharga.Toolkit.StorageConsole.Command
{
    public class RootCommand : RootCommandBase 
    {
        ...
        {
            OutputEvent("Subscription to {0} started by client {1}.", OutputLevel.Information, e.ServerAddress, e.ClientAddress);
        }

        private void SubscriptionHandler_SubscriptionStoppedEvent(object sender, SubscriptionStoppedEventArgs e)
        {
            OutputEvent("Subscription to {0} stopped by {1}. {2}", OutputLevel.Information, e.ServerAddress, e.ClientAddress, e.Reason);
        }

        void SubscriptionHandler_SubscriptionCheckedEvent(object sender, SubscriptionCheckedEventArgs e)
        {
            OutputEvent("Subscription to {0} checked by {1}. {2}", OutputLevel.Information, e.ServerAddress, e.Method, e.Restarted ? "Subscription was restarted." : "Subscription is working.");
        }

        void SubscriptionHandlerSubscriberChangeEvent(object sender, SubscriberChangeEventArgs e)
        {
            OutputEvent("Subscriber to {0} attached or detached. Now {1} subscribers.", OutputLevel.Information, e.ServerAddress, e.SubscriberCount);
        }

        void Instance_CheckOnlineStatusEvent(object sender, CheckOnlineStatusEventArgs e)
        {
            OutputEvent("Checked if the server {0} could be reached. Checked {1} times with no luck.", OutputLevel.Information, e.ServerAddress, e.RetryCount);
        }

        void _subscriptionHandler_SessionCreatedEvent(object sender, SessionCreatedEventArgs e)
        {
            OutputEvent("Session {0} for realm {1} was created.", OutputLevel.Information, e.SessionToken, e.RealmId);
        }
        ...
        {
            OutputEvent("Session {0} has ended.", OutputLevel.Information, e.SessionToken);
        }
    }
}
